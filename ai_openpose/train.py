...
def getModule(prefix=None , begin_epoch=0, batch_size=10,re_init = False,gpus = [2,3]):
    ...
    return model
def train(cmodel,train_data,begin_epoch,end_epoch,batch_size,save_prefix):
    cmodel.init_optimizer(optimizer='sgd', optimizer_params=(('learning_rate', 0.0000004), ))         
...
if __name__ == "__main__":
    prefix  = "../outputs/models/yks_pose"
    cpm_model = getModule(prefix,0,10,True)
    batch_size = 10
    train_data = Ai_data_set(batch_size)
    train(cpm_model,train_data,0,9999,batch_size,prefix)
