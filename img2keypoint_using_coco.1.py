...
class HeatPafCalculator(object):
    ...
    def _getHeatPAF(self,args):
        onedata,model = args
        model.forward(onedata)
    def __init__(self,img_path,models):
    ...
    def begin_calc(self):
        for i in range(len(self.imgs)):
            self._getHeatPAF([self.imgs[i],self.models[i]])
    def get_results(self):
        ...
        return self.img_path,self.oriImg,heatmap_avg,paf_avg
...
if __name__ == "__main__":
    ...
    from img2keypoint_using_coco import parse_heatpaf as phf
    for x,y,z in os.walk(args.images_dir):
        for name in z:
            path = os.path.join(x,name)
            hpaf = HeatPafCalculator(path,models)
            hpaf.begin_calc()
            img_path,oriImg,heatma_av,pafs_av  = hpaf.get_results()
            print(oriImg.shape,heatma_av.shape,pafs_av.shape)
            # phf(img_path,oriImg,heatma_av,pafs_av,args.json_dir)
